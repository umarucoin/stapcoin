# mininode.py - STAP P2P network half-a-node
#
...
#
# NodeConn: an object which manages p2p connectivity to a stap node
...
# Objects that map to stapd objects, which can be serialized/deserialized
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(staphash(r))
            self.hash = encode(staphash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in stapd indicates these should be deserialized as blocks
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=1):
        ...
        print 'MiniNode: Connecting to STAP Node IP # ' + dstaddr + ':' \
            + str(dstport)
