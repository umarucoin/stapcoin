...
*Setup instructions for a Gitian build of STAP Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the STAP
...
- [Building STAP Core](#building-stap-core)
...
Clone the git repositories for STAP Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/umarucoin/stapcoin
```
...
in the STAP Core repository under 'Fetch and build inputs' to install sources which require
...
To build STAP Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the STAP Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/stap/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/umarucoin/stapcoin
...
For example:
```bash
URL=https://github.com/crowning-/stap.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit stap=${COMMIT} --url stap=${URL} ../stap/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit stap=${COMMIT} --url stap=${URL} ../stap/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit stap=${COMMIT} --url stap=${URL} ../stap/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the stap git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../stap/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/stap.git
SIGPATH=/some/root/path/stap-detached-sigs.git

./bin/gbuild --url stap=${BTCPATH},signature=${SIGPATH} ../stap/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/stap-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/stap-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/stap-osx-build.assert
```
...
[stap/gitian.sigs](https://github.com/umarucoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
