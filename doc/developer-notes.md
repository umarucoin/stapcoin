...
STAP Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9698 for connections and services them.
...
However, in open source software such as STAP Core, where everyone uses
...
pay attention to for reviewers of STAP Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: STAP Core uses tinyformat, which is type safe. Leave them out to avoid confusion
