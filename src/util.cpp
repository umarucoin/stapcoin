...
const char * const BITCOIN_CONF_FILENAME = "stap.conf";
const char * const BITCOIN_PID_FILENAME = "stapd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "stap" is a composite category enabling all STAP-related debug output
            if(ptrCategory->count(string("stap"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\STAPCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\STAPCore
    // Mac: ~/Library/Application Support/STAPCore
    // Unix: ~/.stapcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "STAPCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/STAPCore";
#else
    // Unix
    return pathRet / ".stapcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty stap.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override stap.conf
