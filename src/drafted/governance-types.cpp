...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "STAPNetwork") type = STAPNetwork;
    if(strType == "STAPNetworkVariable") type = STAPNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == STAPNetwork) s = "STAPNetwork";
    if(type == STAPNetworkVariable) s = "STAPNetworkVariable";
